...
def handler(event, context):
    ...
    try:
        proc = subprocess.run(f"/home/opam/Synduce/_build/default/bin/Synduce.exe /tmp/tmp.ml --json {message['options']}", stdout=subprocess.PIPE, stderr=subprocess.STDOUT, timeout=870, shell=True)
        status = "FINISHED"
